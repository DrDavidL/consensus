...
with st.sidebar:
    ...
    experts_model_choice = st.toggle(
        "3 AI Experts Model: Use o3-mini",
...
def main():
    ...
    if check_password():
        ...
        deeper_dive = st.sidebar.checkbox(
            "Deeper Dive", help="Check to include more extensive searching.", value=True
        )
            ...
            with col1:
                ...
                if deeper_dive == True:
                        ...
                        # If PubMed articles are found, add to knowledge base and display them
                            ...
                            with st.spinner("Optimizing display of abstracts..."):
                                ...
                                # IF trouble with PubMed formatting, display a link to the search
                                    ...
                                    st.write(
                                        "Trouble formatting retrieved PubMed articles."
                                    )
                                    st.page_link(
                                        "https://pubmed.ncbi.nlm.nih.gov/?term="
                                        + st.session_state.pubmed_search_terms,
                                        label="Click here to try in PubMed",
                                        icon="📚",
                                    )
                                    with st.popover("PubMed Search Terms"):
                                        st.write(
                    ...
                    with st.spinner("Analyzing retrieved content..."):
                        try:
                            ...
                            query_for_rag_response = create_chat_completion(
                                prepare_rag_query_messages,
                                model="o3-mini",
                            )
                            query_for_rag = query_for_rag_response.choices[
                                0
                            ].message.content
                        except Exception as e:
                        ...
                        try:
                            ...
                            citations = [
                                item["metadata"]["url"] for item in semantic_results
                            ]
                        ...
                        try:
                            updated_answer_prompt = rag_prompt2.format(
                                question=original_query, context=semantic_results
                            )
                ...
                ####################### Now we do the quick search without PubMed ############################ Not Deep!
                    ...
                    with st.expander("Retrieved Search Content"):
                        st.write(
                            f"Updated Query: {updated_initial_tavily_query.choices[0].message.content}"
                        )
                        st.write("\n\n")
                    ...
                    updated_initial_question_with_tavily = (
                        original_query
                        + f"Use the following current internet results: {st.session_state.tavily_initial_response}"
                    )
