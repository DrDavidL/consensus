...
with st.sidebar:
    ...
    # st.info("The o3-mini performs well for other options. For more complex synthesis, stay with GPT-4o or use Claude-3.5 Sonnet.")
    
    rag_model_choice = st.radio("Semantic Search Model Options", ["GPT-4o-mini", "GPT-4o","Gemini-1.5"], index=1, help = "Select the RAG model to use for the AI responses.")
    ...
    st.divider()
    experts_model_choice = st.toggle("3 AI Experts Model: Use o3-mini", help = "Toggle to use GPT-4o model for expert responses; otherwise, o3-mini with reasoning.")
...
def display_semantic_search(semantic_search):
    """
    Displays the semantic search results in a user-friendly format in a Streamlit app.

    Args:
        semantic_search (list): A list of dictionaries containing 'context' and 'metadata'.
    """
    st.title("Semantic Search Results")
    
    # Loop through each item in the semantic search results
    for i, item in enumerate(semantic_search, start=1):
        context = item['context']
        url = item['metadata']['url']
        score = item['metadata'].get('score', None)  # Optional: Display score if available
        
        # Display the URL as a clickable link
        st.markdown(f"### Result {i}: [Source]({url})")
        
        # Display the score (if available)
        if score is not None:
            st.markdown(f"**Score:** {score:.4f}")
        
        # Display the context in a clean format
        st.markdown("**Context:**")
        st.write(context)
        
        # Add a divider between results for better readability
        st.markdown("---")
                ...
                if deeper_dive == True:           
                    ...
                    with st.spinner('Analyzing retrieved content...'):
                        try:
                            ...
                            query_for_rag_response = create_chat_completion(prepare_rag_query_messages, model="o3-mini", )
                            query_for_rag = query_for_rag_response.choices[0].message.content
                        except Exception as e:   
                        ...
                        try:
                            updated_answer_prompt = rag_prompt2.format(question=original_query, context = semantic_results)
                            prepare_updated_answer_messages = [
                                                        {'role': 'user', 'content': updated_answer_prompt}]
                            if second_provider == "openai":
                                updated_answer = create_chat_completion(prepare_updated_answer_messages, model=second_model, )    
                            ...
                            st.session_state.rag_response = updated_answer_text    
                    ...
                    full_response = ""
                    if semantic_results:     
                        with st.expander("View Semantic Search Results"):            
