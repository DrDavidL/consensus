            ...
            with col1:
                ...
                if st.session_state.chosen_domain == "medical":
                    pubmed_messages = [
                        {
                            "role": "system",
                            "content": optimize_pubmed_search_terms_system_prompt,
                        },
                        {"role": "user", "content": original_query},
                    ]
                    ...
                    with st.spinner("Optimizing display of abstracts..."):
                        with st.expander(
                            "View PubMed Abstracts Added to Knowledge Base"
                        ):
                            st.warning(
                                f"Note this is a focused PubMed search with {max_results} results added to the database."
                            )
                ...
                with st.spinner("Analyzing retrieved content..."):
                    try:
                        ...
                        answer, citations = app.query(
                            updated_rag_prompt, config=config, citations=True
                        )
                ...
                with container1:
                    st.info(
                        "Preliminary Retrieved Response - See Balanced Expert Persona Opinions"
                    )
        ...
        with col1:
            if first_view == False:
                ...
                if st.session_state.rag_response:
                    st.info(
                        "Preliminary Retrieved Response - See Balanced Expert Opinions"
                    )
                    container1 = st.container(border=True)
                    with container1:
                        st.markdown(st.session_state.rag_response)
