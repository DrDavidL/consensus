    ...
    with st.expander("Settings and About this app"):
        with st.popover("Settings"):
            ...
            max_results = st.number_input(
                "Max results:", min_value=1, max_value=30, value=8
            )
            years_back = st.number_input(
                "Number of years back:", min_value=1, max_value=50, value=3
            )
        ...
        """)
    st.warning(
        "Please try again if you see an error. This app is under rapid iteration."
    )
            ...
            with col1:
                ...
                if st.session_state.chosen_domain == "medical":
                    ...
                    with st.spinner("Adding PubMed abstracts to the knowledge base..."):
                        if articles:
                            try:
                                ...
                                st.error(
                                    "An error occurred while processing the articles. Please try again."
                                )
                        ...
                        if urls:
                            ...
                            for url in urls:
                                try:
                                    ...
                                    logger.error(
                                        f"Connection error for URL {url}: {str(e)}"
                                    )
                    ...
                    if not articles and not urls:
                        st.warning(
                            "No articles or URLs were provided to add to the knowledge base."
                        )
                ...
                with st.spinner("Analyzing retrieved content..."):
                    try:
                        ...
                        updated_rag_prompt = rag_prompt.format(
                            xml_query=updated_rag_query,
                            current_datetime=current_datetime,
                        )
                        answer, citations = app.query(
                            updated_rag_prompt, citations=True
                        )
        ...
        with col2:
            if st.session_state.rag_response:
                if st.button("Assess Response Accuracy"):
                    ...
                    evaluation_prompt = evaluate_response_prompt.format(
                        prior_context=prior_context,
                        prior_answer=st.session_state.rag_response,
                    )
                    try:
                        confirm_response_messages = [
                            {
                                "role": "system",
                                "content": "You are an expert AI evaluator who proceeds step by step and double-checks all your answers since lives may depend on your evaluations.",
                            },
                            {"role": "user", "content": evaluation_prompt},
                        ]
                        confirm_response = create_chat_completion(
                            messages=confirm_response_messages, temperature=0.3
                        )
                ...
                if st.button("Ask 3 AI Experts"):
                    ...
                    with st.spinner("Waiting for experts to respond..."):
                        st.session_state.expert_answers = asyncio.run(
                            get_responses(
                                [expert1_messages, expert2_messages, expert3_messages]
                            )
                        )
            ...
            if first_view == False:
                ...
                if st.session_state.pubmed_search_terms:
                    with st.expander("View PubMed Results Added to Knowledge Base"):
                        ...
                        st.page_link(
                            pubmed_link, label="Click here to view in PubMed", icon="📚"
                        )
                        with st.popover("PubMed Search Terms"):
                            st.write(
                                f"**Search Strategy:** {st.session_state.pubmed_search_terms}"
        ...
        if st.session_state.followup_messages:
            with st.sidebar:
                with st.expander("Followup Conversation"):
                    full_conversation = ""
                    replace_first_user_message(
                        st.session_state.followup_messages,
                        {"role": "user", "content": st.session_state.original_question},
                    )
                    for message in st.session_state.followup_messages:
                        if message["role"] != "system":
                            full_conversation += (
