...
def fetch_api_key():
    ...
    try:
        ...
        if st.session_state.openai_api_key != "":
            ...
            st.warning(
                "Oh, dear friend of mine! It seems your API key has gone astray, hiding in the shadows. Pray, reveal it to me!"
            )
            api_key = st.text_input(
                "Please, whisper your API key into my ears: ", key="warning2"
            )
...
if check_password():
    ...
    st.session_state.model = st.selectbox(
        "Select a model", ["gpt-3.5-turbo-16k", "gpt-4"]
    )
    st.session_state.iteration_limit = st.number_input(
        "Iteration Limit", min_value=1, max_value=10, value=5
    )
    st.session_state.query = st.text_area(
        "Type a natural language math problem (e.g, what is the area of a circle with a radius of 4cm), or expression (24 factorial). Or, even ask me: Create a story problem and solve it!"
    )
    ...
    if st.session_state.message_history is not None:
        if len(st.session_state.message_history) == 50:
            st.write(
                "The message history is getting long. The oldest messages will be summarized. Download now if you need a full record."
            )
        if len(st.session_state.message_history) > 50:
            ...
            summarized_history = gen_response(
                summary_prefix, st.session_state.message_history, st.session_state.model
            )
